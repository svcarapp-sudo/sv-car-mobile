...
import {MyPartBadges} from './MyPartBadges'
import {MyPartCardFooter} from './MyPartCardFooter'
import {MyPartCardMenu} from './MyPartCardMenu'
...
interface MyPartCardItemProps {
    ...
    onLongPress: (partId: string) => void
}

const T = {FRESH: 'جديد', MORE: (n: number) => `+${n} مركبة`}
...
export const MyPartCardItem = (props: MyPartCardItemProps) => {
    const {part, categoryInfo, selectionMode, selected, onToggleSelect, onLongPress, onEdit} = props
    const theme = useAppTheme()
    const vehicles = part.compatibleVehicles ?? []
    const firstVehicle = vehicles[0]
    const isFresh = part.createdAt != null && Date.now() - part.createdAt < FRESH_WINDOW_MS

    const handlePress = () => (selectionMode ? onToggleSelect(part.id) : onEdit(part.id))
                ...
                <View style={styles.body}>
                    <View style={styles.mediaBox}>
                        ...
                        {selectionMode ? (
                            <View
                                style={[
                                    styles.check,
                                    {backgroundColor: selected ? theme.colors.primary : theme.colors.backdrop},
                                ]}>
                                <Icon
                                    source={selected ? 'check' : 'checkbox-blank-circle-outline'}
                                    size={16}
                    ...
                    <View style={styles.info}>
                        <View style={styles.topRow}>
                            ...
                            {!selectionMode && <MyPartCardMenu {...props} />}
                        </View>
                        <Text style={[styles.name, {color: theme.colors.onSurface}]} numberOfLines={2}>
                            {part.name}
                        </Text>
                        <MyPartBadges status={part.status} condition={part.condition} stockQuantity={part.stockQuantity} />
                        {firstVehicle && (
